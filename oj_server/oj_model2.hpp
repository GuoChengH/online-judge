...
namespace ns_model
{
  ...
  public:
    ...
    bool QueryMySql(const std::string &sql, vector<Question> *out)
    {
      ...
      {
        LOG(WARNING) << sql << " 执行错误: " << mysql_error(my) << "\n";
        mysql_close(my);
        return false;
      }
      ...
      if (!res)
      {
        LOG(WARNING) << "获取结果错误: " << mysql_error(my) << "\n";
        mysql_close(my);
        return false;
      }
      ...
      for (int i = 0; i < rows; i++)
      {
        ...
        q.tail = row[4] ? row[4] : "";
        q.cpu_limit = row[5] ? atoi(row[5]) : 0;
        q.mem_limit = row[6] ? atoi(row[6]) : 0;
        q.desc = row[7] ? row[7] : "";
